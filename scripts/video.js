...
const loadCategoriesVideo = (id) =>{
    ...
    .then((res) => res.json())
    .then((data) => {
      // shobaike active class remove 
      removeActiveClass();


      // id er class ke active
...
const displayDetails = (video) =>{
  ...
  <h3 class="font-bold mt-2">Details</h3>
...
const displayVideos = (videos) => {
  ...
  <div class="px-2 py-3 flex gap-2">
    ...
    <button onclick="loadDetails('${video.video_id}')" class="btn btn-sm btn-error text-white mt-2"> Details
    </button>
    </div>
  </div>
        `;
